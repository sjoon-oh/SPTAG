...
#ifndef _SPTAG_EXT_CACHE_MTLFU_H
#define _SPTAG_EXT_CACHE_MTLFU_H
...
#define CACHE_PROMOTION_THRESH  8
...
//         };
        ...
        protected:
            ...
            std::array<std::unique_ptr<CacheLfuCore>, CACHE_CORRLFU_MAX_LEVEL> m_leveledLru;
        ...
        public:
            CacheMTLfu(const size_t p_capacity, bool p_enableLock = false) noexcept
                : m_capacity(p_capacity), m_cachedShared(new DATA_CACHED_T())
            {
                double leveledSize = (p_capacity * 0.60);
                m_leveledLru[0].reset(new CacheLfuCore(size_t(leveledSize), m_cachedShared)); // Set base size
                
                leveledSize = (p_capacity * 0.20);
                m_leveledLru[1].reset(new CacheLfuCore(size_t(leveledSize), m_cachedShared)); // Set second level size
                
                int leftLevels = CACHE_CORRLFU_MAX_LEVEL - 2;
                for (int lev = 2; lev < CACHE_CORRLFU_MAX_LEVEL; lev++)
                {
                    m_leveledLru[lev].reset(
                        new CacheLfuCore(size_t((p_capacity * 0.20) / leftLevels), m_cachedShared));
                }
            ...
            {                  
                ...
                // std::cout << "Hit Counts:" << localHits << std::endl;
                
                size_t evictCounts = 0;
                if (localHits > CACHE_PROMOTION_THRESH)
                {
                    for (auto& index: indexHit)
                    {
                        // 
                        // Case when hit. 
                ...
                evictCounts += (m_leveledLru[0]->evictOverflows(totalSize));
