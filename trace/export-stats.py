...
def analysis_cache_trace_csv(filename):
    ...
    # Must extract over 94162

    hit_counts = np.sort(hit_counts[94162:])
    miss_counts = np.sort(miss_counts[94162:])
    evict_counts = np.sort(evict_counts[94162:])
    curr_size = np.sort(curr_size[94162:])
    per_query_hit_ratio = np.sort(per_query_hit_ratio[94162:])
